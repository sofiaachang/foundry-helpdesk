...
- Ontology entities created through the API need three UI steps before the service can read them: **Add new data store** on each object type's Datasources page (otherwise `OntologySyncingObjectTypes` forever), **Searchable** on every property the service filters or sorts on (otherwise `PropertiesNotSearchable`), and the object types, links, and Action added to the Developer Console app's data scope (otherwise a 404 on a type that exists). Redo the scope after recreating types.
- The Action is `create-help-desk-issue` with parameters `reportedByUserId` and `assignedTeamId`; the service maps its contract names onto them through `FOUNDRY_ONTOLOGY_NAMES` on Railway.
- ElevenLabs CLI (1.2.0) rewrites `agents.json`/`tools.json`/`tests.json` on push and drops the `name` key; `render.ts` matches by config path. Tool-call unit tests need JSONPath parameter paths (`$.title`), not bare names. The OAuth token lacks `webhooks_write`; webhooks are UI work.
