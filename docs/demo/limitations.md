...
15. **Latency is measured at the service, not at the ear.** The post-call webhook payload on this plan carried no time-to-first-byte or first-sentence fields, so the only numbers are the service's own tool timings: 0.3 to 0.9 s per tool on the dry run (verify 342 ms, status 677 ms, team queue 889 ms, similar search 315 ms, create 829 ms). Time to first audio adds model and text-to-speech latency on top.
