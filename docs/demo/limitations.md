...
18. **Ontology authoring through the CLI has gaps.** The object types, properties, and links were created with `pltr` (0.29.1 from the local repo), but the index for a new object type takes several minutes to become readable, value types cannot be attached from the CLI, title keys cannot be set, and the Action type's internal definition format is undocumented (Foundry's validator rejects a best-effort document without detail), so the Action, value types, and title keys are created in Ontology Manager by hand.
19. **What zap permitted versus what the plan assumed**: no client-credentials grant (assumed available), network ingress allowlist (not assumed), backend-service applications unavailable. Object Storage v2 availability: pending from U3.
